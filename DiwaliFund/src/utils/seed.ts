import type { Member, Membership, Payment, Scheme } from '../types';
import { periods } from './fund';

export function createSeed(): {
  members: Member[];
  schemes: Scheme[];
  ms: Membership[];
  payments: Payment[];
} {
  const names = [
    'Ravi Kumar',
    'Kumar',
    'Suresh',
    'Mani',
    'Priya',
    'Arun',
    'Vijay',
    'Bala',
    'Dinesh',
    'Mohan',
    'Raj',
    'Selvam',
    'Karthik',
    'Sathish',
    'Prakash',
    'Gokul',
  ];
  const members: Member[] = names.map((n, i) => ({
    id: 'm' + i,
    name: n,
    phone: '98' + String(41000000 + i * 7919).padStart(8, '0'),
  }));
  const schemes: Scheme[] = [
    {
      id: 's1',
      name: 'Diwali Weekly 100',
      type: 'WEEKLY',
      unit: 100,
      start: '2026-01-05',
      end: '2026-11-08',
      color: '#E9A23B',
    },
    {
      id: 's2',
      name: 'Diwali Weekly 200',
      type: 'WEEKLY',
      unit: 200,
      start: '2026-01-05',
      end: '2026-11-08',
      color: '#1D4ED8',
    },
    {
      id: 's3',
      name: 'Diwali Monthly 500',
      type: 'MONTHLY',
      unit: 500,
      start: '2026-01-05',
      end: '2026-11-08',
      color: '#7C3AED',
    },
    {
      id: 's4',
      name: 'Diwali Monthly 1000',
      type: 'MONTHLY',
      unit: 1000,
      start: '2026-01-05',
      end: '2026-11-08',
      color: '#059669',
    },
  ];
  const links: [number, number, number][] = [
    [0, 0, 2],
    [0, 1, 1],
    [0, 2, 3],
    [1, 0, 3],
    [2, 0, 1],
    [2, 3, 1],
    [3, 1, 2],
    [4, 2, 1],
    [5, 0, 1],
    [5, 2, 2],
    [6, 1, 1],
    [6, 3, 1],
    [7, 0, 2],
    [8, 2, 1],
    [9, 0, 1],
    [9, 1, 1],
    [10, 3, 2],
    [11, 0, 1],
    [12, 1, 1],
    [12, 2, 1],
    [13, 0, 2],
    [13, 3, 1],
    [14, 2, 2],
    [15, 0, 1],
    [15, 1, 2],
  ];
  const fr = [
    0.85, 1, 0.7, 1, 0.6, 0.95, 1, 0.5, 0.8, 1, 0.75, 0.9, 1, 0.65, 1, 0.55, 0.88, 1, 0.7, 1,
    0.92, 0.6, 1, 0.8, 0.5,
  ];
  const ms: Membership[] = [];
  const payments: Payment[] = [];
  let pid = 0;
  links.forEach(([mi, si, units], i) => {
    const s = schemes[si];
    const id = 'ms' + i;
    ms.push({ id, memberId: 'm' + mi, schemeId: s.id, units });
    const per = periods(s);
    const elapsed = per.elapsed;
    const k = Math.round(elapsed * fr[i]);
    for (let j = 0; j < k; j++) {
      const d = new Date(s.start);
      if (s.type === 'WEEKLY') d.setDate(d.getDate() + j * 7);
      else d.setMonth(d.getMonth() + j);
      d.setDate(d.getDate() + ((j + mi) % 4));
      payments.push({
        id: 'p' + pid++,
        msId: id,
        amount: s.unit * units,
        date: d.toISOString().slice(0, 10),
        method: (j + mi) % 3 === 0 ? 'CASH' : 'GPAY',
      });
    }
    if (fr[i] < 1 && i % 3 === 0) {
      payments.push({
        id: 'p' + pid++,
        msId: id,
        amount: (s.unit * units) / 2,
        date: '2026-09-16',
        method: 'GPAY',
      });
    }
  });
  return { members, schemes, ms, payments };
}
